...
const validateName = (name: string): string => {
  ...
  if (!regex.test(name)) return "Nama tidak boleh mengandung simbol atau angka";
  return "";
};
...
const validateEmail = (email: string): string => {
  ...
  if (!regex.test(email)) return "Email tidak valid.";
  return "";
};

const validateDisabilityType = (type: string): string => {
  if (!type) return "Jenis disabilitas harus dipilih.";
  return "";
};
...
export const RegisterPage: React.FC<RegisterPageProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const role = formData.disability_type === DisabilityType.MENTOR 
      ? UserRole.MENTOR 
      : UserRole.STUDENT;
