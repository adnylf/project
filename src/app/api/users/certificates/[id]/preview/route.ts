...
// GET /api/users/certificates/[id]/preview - Get rendered certificate HTML
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const mentorTemplateConfig = await prisma.systemSetting.findUnique({
      where: { key: `mentor_template_${certificate.course.mentor_id}` },
    });

    // Get mentor's signature
    const mentorSignatureConfig = await prisma.systemSetting.findUnique({
      where: { key: `mentor_signature_${certificate.course.mentor_id}` },
    });
    ...
    const html = templateContent
      ...
      .replace(/\{\{MENTOR_NAME\}\}/g, certificate.course.mentor.user.full_name)
